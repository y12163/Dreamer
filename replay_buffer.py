...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        batch_obs = np.empty((self.batch_size, self.seq_len, *self.obs_shape), dtype=np.uint8)
        ...
        for i in range(self.batch_size):
            ...
            batch_obs[i] = episode['obs'][start_idx:start_idx + self.seq_len]
        ...
        batch_obs = torch.from_numpy(batch_obs).float()  # [batch_size, seq_len, *obs_shape]
        ...
        batch_obs = batch_obs.permute(1, 0, *range(2, batch_obs.ndim)).contiguous()
